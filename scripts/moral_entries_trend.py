...
def render_one(xlsx_path: str | Path, out_path: str | Path) -> None:
    ...
    for r in range(2, ws.max_row + 1):
        d_v = ws.cell(r, idx["周起"]).value
        c_v = ws.cell(r, idx["德育分数录入数"]).value
        if d_v is None:
            continue
        d = parse_date(d_v)
        try:
    ...
    y = [int(by_week[d]) for d in weeks]

    init_fonts()
    plt.figure(figsize=(14, 6))
    plt.plot(x, y, linewidth=2.2, color="#F59E0B", label="德育分数录入数（合计）")
    plt.title("每周德育分数录入数趋势（近13个月）\n数据截至 2026-03-13")
    plt.xlabel("周起始日")
    plt.ylabel("录入数")
