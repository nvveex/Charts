...
def render_one(xlsx_path: str | Path, out_path: str | Path) -> None:
    ...
    for r in range(2, ws.max_row + 1):
        ...
        cnt_v = ws.cell(r, idx["问卷提交数"]).value
        if week_v is None:
            continue
    ...
    values = [int(by_week[w]) for w in weeks]
    x = [datetime.strptime(w, "%Y-%m-%d") for w in weeks]

    init_fonts()
    plt.figure(figsize=(14, 6))
    plt.plot(x, values, linewidth=2.2, color="#06B6D4", label="问卷提交数（合计）")
    plt.title("每周问卷提交数趋势（近一年）\n数据截至 2026-03-13")
    plt.xlabel("周起始日")
    plt.ylabel("提交数")
